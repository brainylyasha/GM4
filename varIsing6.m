function [E, D, M, L] = varIsing6_2(H, J, betaAll, opt_params)
% ВХОД
...
% L — нижние границы для логарифмов нормировочных констант для каждой температуры, вектор-строка длины \beta_0.
    ...
    edge_vert = reshape(repmat((1:(v - 1))', 1, h) + repmat((0:(h - 1)) * v, v - 1, 1), ...
            (v - 1) * h, 1);
    edge_hor = (1:(N - v))';
    edge_diag = reshape(repmat((1:(v - 1))', 1, h - 1) + repmat((0:(h - 2)) * v, v - 1, 1), ...
            (v - 1) * (h - 1) , 1);
    edge = [edge_vert, edge_vert + 1; ...
            edge_hor, edge_hor + v; ...
            edge_diag, edge_diag + (v + 1)];
    ...
    for start = 1:num_start
        ...
        for iter = 1:max_iter
            ...
            L_new = betaAll .* (sum(H_rep .* (2 * q - 1), 1) + ...
